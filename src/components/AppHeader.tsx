    ...
    <>
      <div className="h-10 border-b flex items-center justify-between px-3 shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          {title && <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">{title}</span>}
        </div>
        <button
        ...
        >
          ...
          <span className="text-[11px] text-muted-foreground font-mono">SYS</span>
        </button>
      </div>
          ...
          <div className="w-80 bg-background border-l h-full flex flex-col animate-slide-in-right" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-1">
              {systemHealth.issues.map((issue, i) => (
                <div key={i} className="flex items-start gap-2 py-1.5 border-b border-border/30 last:border-0 group">
                  {issue.severity === "error" ? <AlertTriangle className="h-3.5 w-3.5 text-destructive shrink-0 mt-px" /> :
                   issue.severity === "warning" ? <AlertTriangle className="h-3.5 w-3.5 text-warning shrink-0 mt-px" /> :
                   <Check className="h-3.5 w-3.5 text-success shrink-0 mt-px" />}
                  <div className="flex-1 min-w-0">
                    <span className="text-[12px]">{issue.label}</span>
                    {issue.action && (
                      <button className="block text-[11px] text-accent hover:text-accent/80 mt-0.5 flex items-center gap-1">
            ...
            <div className="border-t p-2">
              <button className="w-full flex items-center justify-center gap-2 py-1.5 rounded bg-secondary text-[12px] text-foreground hover:bg-secondary/80 transition-colors font-medium">
