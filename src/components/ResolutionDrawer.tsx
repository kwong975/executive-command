...
export function ResolutionDrawer({ item, onClose, onAction }: ResolutionDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end" onClick={onClose}>
      ...
      {/* Drawer panel */}
      <div
        className="relative w-[460px] max-w-full h-full bg-background border-l border-border/50 shadow-2xl flex flex-col animate-in slide-in-from-right duration-200"
      ...
      >
            ...
            <div className="space-y-2.5">
              {item.objective && (
                <div className="flex items-start gap-3">
                  <span className="text-xs text-muted-foreground shrink-0 w-16 pt-0.5">Objective</span>
                  <span className="text-sm text-foreground/80">{item.objective}</span>
                </div>
              )}
              {item.matterTitle && item.type !== "matter" && (
                <div className="flex items-start gap-3">
                  <span className="text-xs text-muted-foreground shrink-0 w-16 pt-0.5">Matter</span>
                  <span className="text-sm text-foreground/80">{item.matterTitle}</span>
                </div>
              )}
              {item.dueDate && (
                <div className="flex items-start gap-3">
                  <span className="text-xs text-muted-foreground shrink-0 w-16 pt-0.5">Due</span>
                  <span className="text-sm text-foreground/80">{item.dueDate}</span>
                </div>
              )}
              {item.participants && item.participants.length > 0 && (
                <div className="flex items-start gap-3">
                  <span className="text-xs text-muted-foreground shrink-0 w-16 pt-0.5">People</span>
                  <div className="flex flex-wrap gap-1">
                    {item.participants.map(p => (
                      <span key={p} className="text-xs px-1.5 py-0.5 rounded-md bg-secondary text-muted-foreground font-mono">
                        {p}
                      </span>
                    ))}
                  </div>
              ...
              {item.recentActivity && (
                <div className="flex items-start gap-3">
                  <span className="text-xs text-muted-foreground shrink-0 w-16 pt-0.5">Activity</span>
                  <span className="text-sm text-muted-foreground">{item.recentActivity}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {/* Actions footer */}
        <div className="px-5 py-4 border-t border-border/50 shrink-0 bg-card/50">
          <span className="text-xs uppercase tracking-widest text-muted-foreground font-semibold mb-2.5 block">
            Actions
          </span>
          <div className="flex flex-wrap gap-2">
            {actions.map(action => (
              <button
                ...
                className={cn(
                  "flex items-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium transition-all duration-150 h-8",
                  action.primary
