...
export function CommandDrawer({ agent, onClose }: { agent: Agent | null; onClose: () => void }) {
  ...
  return (
    <div className="w-[300px] border-l border-border/50 flex flex-col shrink-0 bg-background">
      {/* Header */}
      <div className="h-11 flex items-center justify-between px-3 border-b border-border/50 shrink-0">
        <div className="flex items-center gap-2">
          <Terminal className="h-3.5 w-3.5 text-accent" />
          <span className="text-sm font-mono font-semibold">{agent.name}</span>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
        {messages.map(msg => (
          <div key={msg.id}>
            {msg.role === "system" ? (
              <div className="bg-secondary/50 rounded-md px-3 py-2 text-sm leading-relaxed border border-border/30">
                <div className="flex items-center gap-2">
      ...
      {/* Quick commands */}
      <div className="px-3 py-2 border-t border-border/30 flex flex-wrap gap-1">
        {quickCommands.map(cmd => (
          <button
            key={cmd}
            onClick={() => send(cmd)}
            className="text-xs px-1.5 py-1 rounded-md bg-secondary text-muted-foreground hover:text-foreground hover:bg-secondary/80 transition-all duration-100 flex items-center gap-1 font-medium"
          >
      ...
      <div className="border-t border-border/50 p-2.5 shrink-0">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 text-sm bg-secondary rounded-md px-3 py-2 outline-none placeholder:text-muted-foreground/60 focus:ring-1 focus:ring-accent/50 font-mono transition-shadow"
          />
          <button
            onClick={() => send()}
            disabled={!input.trim()}
            className="p-2 rounded-md bg-accent text-accent-foreground hover:bg-accent/90 disabled:opacity-20 transition-all shadow-sm h-8 w-8 flex items-center justify-center"
          >
