    ...
    return (
      <AppLayout title="Strategy">
        <div className="flex items-center justify-center h-full text-muted-foreground gap-2">
          <Loader2 className="h-4 w-4 animate-spin" /><span className="text-sm">Loading strategy...</span>
        </div>
      </AppLayout>
    );
  }
  ...
  return (
    <AppLayout title="Strategy">
      <div className="flex flex-col flex-1 overflow-hidden">
        {/* BU Tabs */}
        <div className="border-b border-border/50 px-4 flex items-center gap-1 shrink-0">
          {BU_ORDER.map(bu => {
            ...
            return (
              ...
              >
                {BU_LABELS[bu] || bu}
                {goalCount > 0 && <span className="ml-1.5 text-xs font-mono text-muted-foreground tabular-nums">{goalCount}</span>}
              </button>
            );
          })}
          <div className="ml-auto flex items-center gap-2">
            {unlinkedMatters > 0 && (
              <span className="flex items-center gap-1.5 text-xs text-warning mr-2">
                <AlertTriangle className="h-3 w-3" />
                {unlinkedMatters} unlinked
              </span>
            )}
            {seedResult && (
              <span className="text-xs text-success mr-2">{seedResult}</span>
            )}
            <button onClick={() => setShowAddBU(true)} className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground bg-secondary rounded-md transition-colors h-7">
              <Plus className="h-3 w-3" />BU Goal
            </button>
            <button onClick={() => setShowAddDept(true)} className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground bg-secondary rounded-md transition-colors h-7">
              <Building2 className="h-3 w-3" />Dept Goal
            </button>
            <button onClick={handleSeed} disabled={runSeed.isPending} className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground bg-secondary rounded-md transition-colors disabled:opacity-50 h-7">
        ...
        {currentBU?.source_file && (
          <div className="border-b border-border/30 px-4 py-2 flex items-center gap-4 text-xs text-muted-foreground shrink-0 bg-secondary/30">
          ...
          {companyGoals.length > 0 && (
            <section className="mb-6">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-accent">Long-Term Direction</span>
              </div>
              {companyGoals.map(g => (
          ...
          {buGoals.length > 0 && (
            <section className="mb-6">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Annual Goals</span>
              </div>
              {buGoals.map(g => (
                <GoalCard key={g.id as string} goal={g} expanded={expanded[g.id as string]} onToggle={() => toggleExpand(g.id as string)} navigate={navigate} onUnlink={(mid) => unlinkMatter.mutate(mid)} onArchive={() => handleArchive(g.id as string)} onLink={() => setLinkingGoalId(g.id as string)} />
              ))}
          ...
          {Object.keys(deptsByCode).length > 0 && (
            <section className="mb-6">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Department Goals</span>
              </div>
              {Object.entries(deptsByCode).map(([code, depts]) => (
                <div key={code} className="mb-4">
                  <div className="text-xs font-semibold text-muted-foreground mb-1.5 pl-1 uppercase tracking-widest">
      ...
      />
      {showAddDept && (
        <div className="fixed inset-0 bg-background/60 backdrop-blur-sm z-50 flex items-center justify-center" onClick={() => { setShowAddDept(false); setDeptCode(""); setDeptName(""); }}>
          <div className="bg-card border border-border/50 rounded-lg shadow-2xl w-[400px] overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-4 py-3.5 border-b border-border/50">
              <h3 className="text-sm font-semibold text-foreground">Add Department Goal</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Add to {BU_LABELS[activeBU] || activeBU}</p>
            </div>
            <div className="px-4 py-3.5 space-y-3">
              <div>
                <label className="text-xs text-muted-foreground mb-1 block font-medium uppercase tracking-widest">Department code</label>
                <input type="text" value={deptCode} onChange={e => setDeptCode(e.target.value)} placeholder="e.g. HRC, FA, DTO"
                  className="w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-accent/50" autoFocus />
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block font-medium uppercase tracking-widest">Department name</label>
                <input type="text" value={deptName} onChange={e => setDeptName(e.target.value)} placeholder="e.g. Human Resources"
                  className="w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-accent/50" />
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-border/50 bg-secondary/20">
              <button onClick={() => { setShowAddDept(false); setDeptCode(""); setDeptName(""); }} className="px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors h-8">Cancel</button>
              <button onClick={() => handleAddDeptGoal("")} disabled={!deptCode.trim() || !deptName.trim()}
                className="px-4 py-1.5 text-sm font-medium rounded-md bg-accent text-accent-foreground hover:bg-accent/90 disabled:opacity-50 transition-colors shadow-sm h-8">Create</button>
            </div>
          </div>
        </div>
      )}
...
function GoalCard({ goal, expanded, onToggle, navigate, onUnlink, onArchive, onLink, compact }: {
  ...
  return (
    <div className={`border rounded-lg mb-2 transition-all duration-100 ${
      hasProblems ? "border-destructive/30" : "border-border/40"
    } ${isDraft ? "opacity-70 border-dashed" : ""}`}>
      <div className="flex items-center justify-between px-3.5 py-2.5 cursor-pointer hover:bg-secondary/30 transition-colors group" onClick={onToggle}>
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          ...
          {isDraft && <span className="text-xs px-2 py-0.5 rounded-md bg-accent/15 text-accent font-semibold uppercase shrink-0 border border-accent/20">Draft</span>}
          {blocked > 0 && <span className="text-xs px-1.5 py-0.5 rounded-md bg-destructive/15 text-destructive font-semibold shrink-0 border border-destructive/20">{blocked} blocked</span>}
          {atRisk > 0 && <span className="text-xs px-1.5 py-0.5 rounded-md bg-warning/15 text-warning font-semibold shrink-0 border border-warning/20">{atRisk} at risk</span>}
        </div>
        <div className="flex items-center gap-3 shrink-0">
          {metric && target && (
            <span className="text-xs text-muted-foreground"><span className="font-mono tabular-nums">{target}</span> · {metric}</span>
          )}
          {owner && <span className="text-xs font-mono text-muted-foreground">{owner}</span>}
          {matterCount > 0
            ? <span className="text-xs font-mono text-muted-foreground bg-secondary px-1.5 py-0.5 rounded-md tabular-nums">{matterCount}</span>
            : <span className="text-xs text-warning font-semibold">gap</span>}
      ...
      {expanded && matters.length > 0 && (
        <div className="border-t border-border/30 px-3 py-1.5">
          {matters.map((m) => (
            <DenseRow key={m.id as string} onClick={() => navigate("/matters")}>
              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                ...
                {(m.overdue as number) > 0 && <span className="text-xs px-1.5 py-0.5 rounded-md bg-destructive/15 text-destructive font-semibold tabular-nums">{m.overdue as number}</span>}
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <span className="text-xs font-mono text-muted-foreground">{m.owner as string}</span>
          ...
          <button onClick={(e) => { e.stopPropagation(); onLink(); }} className="mt-1.5 text-xs text-muted-foreground hover:text-foreground flex items-center gap-1 px-1 py-1 rounded-md hover:bg-secondary transition-colors font-medium">
      ...
      {expanded && matters.length === 0 && !isDraft && (
        <div className="border-t border-border/30 px-3.5 py-2.5 text-xs text-warning flex items-center justify-between">
      ...
      {expanded && isDraft && (
        <div className="border-t border-border/30 px-3.5 py-2.5 text-xs text-muted-foreground flex items-center gap-1.5">
