    ...
    return (
      <AppLayout title="Strategy">
        <div className="flex items-center justify-center h-full text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin mr-2" /><span className="text-[13px]">Loading strategy...</span>
        </div>
      </AppLayout>
    );
  }
    ...
    <AppLayout title="Strategy">
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="border-b px-4 flex items-center gap-1 shrink-0">
          {BU_ORDER.map(bu => {
            ...
            return (
              ...
              >
                {BU_LABELS[bu] || bu}
                {goalCount > 0 && <span className="ml-1.5 text-[11px] font-mono text-muted-foreground">{goalCount}</span>}
              </button>
            );
          })}
          <div className="ml-auto flex items-center gap-2">
            {unlinkedMatters > 0 && (
              <span className="flex items-center gap-1 text-[12px] text-warning mr-2">
                <AlertTriangle className="h-3 w-3" />
                {unlinkedMatters} unlinked
              </span>
            )}
            {seedResult && (
              <span className="text-[12px] text-success mr-2">{seedResult}</span>
            )}
            <button onClick={() => setShowAddBU(true)} className="flex items-center gap-1 px-2 py-1 text-[12px] font-medium text-muted-foreground hover:text-foreground bg-secondary rounded transition-colors">
              <Plus className="h-3 w-3" />BU Goal
            </button>
            <button onClick={() => setShowAddDept(true)} className="flex items-center gap-1 px-2 py-1 text-[12px] font-medium text-muted-foreground hover:text-foreground bg-secondary rounded transition-colors">
              <Building2 className="h-3 w-3" />Dept Goal
            </button>
            <button onClick={handleSeed} disabled={runSeed.isPending} className="flex items-center gap-1 px-2 py-1 text-[12px] font-medium text-muted-foreground hover:text-foreground bg-secondary rounded transition-colors disabled:opacity-50">
        ...
        {currentBU?.source_file && (
          <div className="border-b border-border/20 px-4 py-1.5 flex items-center gap-4 text-[12px] text-muted-foreground shrink-0 bg-secondary/10">
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto px-4 py-3 max-w-4xl">
          {companyGoals.length > 0 && (
            <section className="mb-5">
              <div className="flex items-center gap-2 mb-2">
                <Target className="h-3.5 w-3.5 text-accent" />
                <span className="text-[12px] font-semibold uppercase tracking-widest text-accent">Long-Term Direction</span>
              </div>
              {companyGoals.map(g => (
          ...
          {buGoals.length > 0 && (
            <section className="mb-5">
              <div className="flex items-center gap-2 mb-2">
                <Target className="h-3.5 w-3.5 text-foreground/50" />
                <span className="text-[12px] font-semibold uppercase tracking-widest text-muted-foreground">Annual Goals</span>
              </div>
              {buGoals.map(g => (
                <GoalCard key={g.id as string} goal={g} expanded={expanded[g.id as string]} onToggle={() => toggleExpand(g.id as string)} navigate={navigate} onUnlink={(mid) => unlinkMatter.mutate(mid)} onArchive={() => handleArchive(g.id as string)} onLink={() => setLinkingGoalId(g.id as string)} />
              ))}
          ...
          {Object.keys(deptsByCode).length > 0 && (
            <section className="mb-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[12px] font-semibold uppercase tracking-widest text-muted-foreground">Department Goals</span>
              </div>
              {Object.entries(deptsByCode).map(([code, depts]) => (
                <div key={code} className="mb-3">
                  <div className="text-[12px] font-semibold text-muted-foreground mb-1 pl-1">
      ...
      />
      {showAddDept && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => { setShowAddDept(false); setDeptCode(""); setDeptName(""); }}>
          <div className="bg-card border border-border rounded-lg shadow-xl w-[400px] overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-4 py-3 border-b">
              <h3 className="text-sm font-semibold">Add Department Goal</h3>
              <p className="text-[13px] text-muted-foreground mt-0.5">Add to {BU_LABELS[activeBU] || activeBU}</p>
            </div>
            <div className="px-4 py-3 space-y-3">
              <div>
                <label className="text-[12px] text-muted-foreground mb-1 block">Department code</label>
                <input type="text" value={deptCode} onChange={e => setDeptCode(e.target.value)} placeholder="e.g. HRC, FA, DTO"
                  className="w-full px-3 py-2 text-sm border border-border rounded-md bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent" autoFocus />
              </div>
              <div>
                <label className="text-[12px] text-muted-foreground mb-1 block">Department name</label>
            ...
            <div className="flex items-center justify-end gap-2 px-4 py-3 border-t bg-secondary/10">
              <button onClick={() => { setShowAddDept(false); setDeptCode(""); setDeptName(""); }} className="px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground">Cancel</button>
              <button onClick={() => handleAddDeptGoal("")} disabled={!deptCode.trim() || !deptName.trim()}
                className="px-4 py-1.5 text-sm font-medium rounded-md bg-accent text-accent-foreground hover:bg-accent/90 disabled:opacity-50">Create</button>
            </div>
          </div>
        </div>
      )}
...
function GoalCard({ goal, expanded, onToggle, navigate, onUnlink, onArchive, onLink, compact }: {
  ...
  return (
    <div className={`border border-border/30 rounded-lg mb-2 ${hasProblems ? "border-destructive/30" : ""} ${isDraft ? "opacity-75 border-dashed" : ""}`}>
      <div className="flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-secondary/20 transition-colors group" onClick={onToggle}>
        <div className="flex items-center gap-2 min-w-0 flex-1">
          {matterCount > 0
            ...
            : <span className="w-3" />}
          <span className={`${compact ? "text-[13px]" : "text-sm"} font-medium truncate`}>{title}</span>
          {isDraft && <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent/15 text-accent font-semibold uppercase shrink-0">Draft</span>}
          {blocked > 0 && <span className="text-[11px] px-1.5 py-0.5 rounded bg-destructive/15 text-destructive font-medium shrink-0">{blocked} blocked</span>}
          {atRisk > 0 && <span className="text-[11px] px-1.5 py-0.5 rounded bg-warning/15 text-warning font-medium shrink-0">{atRisk} at risk</span>}
        </div>
        <div className="flex items-center gap-3 shrink-0">
          {metric && target && (
            <span className="text-[12px] text-muted-foreground"><span className="font-mono">{target}</span> · {metric}</span>
          )}
          {owner && <span className="text-[12px] font-mono text-muted-foreground">{owner}</span>}
          {matterCount > 0
            ? <span className="text-[11px] font-mono text-muted-foreground bg-secondary px-1.5 py-0.5 rounded">{matterCount}</span>
            : <span className="text-[11px] text-warning">gap</span>}
          <button onClick={(e) => { e.stopPropagation(); onArchive(); }} className="p-1 rounded hover:bg-destructive/15 text-muted-foreground/30 hover:text-destructive opacity-0 group-hover:opacity-100 transition-all" title="Archive goal">
      ...
      {expanded && matters.length > 0 && (
        <div className="border-t border-border/20 px-3 py-1.5">
          {matters.map((m) => (
            <DenseRow key={m.id as string} onClick={() => navigate("/matters")}>
              <div className="flex items-center gap-2 min-w-0 flex-1">
                ...
                {(m.overdue as number) > 0 && <span className="text-[11px] px-1 py-0.5 rounded bg-destructive/15 text-destructive font-medium">{m.overdue as number}</span>}
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <span className="text-[12px] font-mono text-muted-foreground">{m.owner as string}</span>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <InlineAction icon={<ArrowRight className="h-3 w-3" />} onClick={() => navigate("/matters")} />
                  <button onClick={(e) => { e.stopPropagation(); onUnlink(m.id as string); }} className="p-1 rounded hover:bg-destructive/15 text-muted-foreground hover:text-destructive">
                    <X className="h-3 w-3" />
                  </button>
                </div>
              </div>
          ...
          <button onClick={(e) => { e.stopPropagation(); onLink(); }} className="mt-1 text-[12px] text-muted-foreground hover:text-foreground flex items-center gap-1 px-1 py-0.5 rounded hover:bg-secondary transition-colors">
      ...
      {expanded && matters.length === 0 && !isDraft && (
        <div className="border-t border-border/20 px-3 py-2 text-[12px] text-warning flex items-center justify-between">
      ...
      {expanded && isDraft && (
        <div className="border-t border-border/20 px-3 py-2 text-[12px] text-muted-foreground flex items-center gap-1.5">
